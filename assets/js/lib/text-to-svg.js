...
class TextToSVG {
    ...
    getWidth(text, options) {
        ...
        for (let i = 0; i < glyphs.length; i++) {
            ...
            if (kerning && i < glyphs.length - 1) {
                const kerningValue = this.font.getKerningValue(
                    glyph,
                    glyphs[i + 1],
                );
    ...
    getDebugSVG(text, options = {}) {
        ...
        const box = {
            width:
                Math.max(metrics.x + metrics.width, 0) - Math.min(metrics.x, 0),
            height:
                Math.max(metrics.y + metrics.height, 0) -
                Math.min(metrics.y, 0),
        };
        const origin = {
